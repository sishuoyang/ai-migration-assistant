...
Queries dialog is the source of truth — if the partner edited the queries
since step 1, the new set is what you see here.)

```sql
{olap_queries}
```

Translate each query to ClickHouse SQL **in chat** (this is pure reasoning;
no tool calls required for the rewrite itself). Helpful mappings:

- `CURRENT_TIMESTAMP()` → `now()`
- `DATE_TRUNC('month', x)` → `toStartOfMonth(x)`
- `LISTAGG` → `arrayStringConcat(groupArray(...))`
- VARIANT field access `x:field` → `JSONExtractString(x, 'field')` (or
  `x.field` if the column is typed `JSON`)
