...
**server-side execution time** — Snowflake `INFORMATION_SCHEMA.QUERY_HISTORY`
(`EXECUTION_TIME`), ClickHouse `X-ClickHouse-Summary.elapsed_ns` — so
the comparison is network-neutral. Wall-clock is also recorded as a
secondary diagnostic; the dashboard surfaces it under the primary
number when network RTT adds materially to the request.
...
Benchmarker(
...
).benchmark(queries=[
    {
        ...
        "source_sql": "<original Snowflake SQL from step 4>",
...
- Captures **server-side execution time** for `source_sql` on Snowflake
  and `target_sql` on ClickHouse Cloud (engine-reported, network-neutral);
  also records the wall-clock bracket for transparency.
...
longer than that on the source, raise it.
