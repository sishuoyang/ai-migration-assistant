...
Benchmarker(
...
).benchmark(queries=[
    {
        ...
        "source_sql": "<original BigQuery SQL from {olap_queries}>",
...
- Wall-clocks `source_sql` against BigQuery and `target_sql` against
...
Dispatch + one tail (raise `max_wait_seconds` for long source queries):
...
query where the target was slower than expected or the speedup is
weak (<3×), inspect:

- Was the target table's `ORDER BY` aligned with this query's
  predicate? If not, propose a **PROJECTION** with the right sort key.
- Is the time filter sargable? If it's wrapped in a function, propose a
  computed column or rewrite.
- For aggregation-heavy queries (especially daily/monthly rollups),
  propose an **AggregatingMergeTree** materialised view with
  `groupArrayState` / `sumState` / `avgState` aggregates plus an
  explicit backfill INSERT.
- For `JSONExtractString(order_metadata, '<key>')` queries: if a
  particular key is hit in WHERE / GROUP BY repeatedly, propose a
  computed materialised column (`ALTER TABLE … ADD COLUMN payment_method
  LowCardinality(String) MATERIALIZED JSONExtractString(...)`).
